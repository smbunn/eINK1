...
public:
	...
	// do_refresh=false draws into the frame buffer only; the caller triggers
	// a single update()/display() afterwards instead of one per image.
	void dis_img_Partial_Refresh(uint8_t x, uint8_t y, uint8_t w, uint8_t h, const unsigned char *img, bool rotated = false, bool y_mirror = false, bool do_refresh = true)
	{
